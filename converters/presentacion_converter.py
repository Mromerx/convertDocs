...
def convert(input_path: str, output_format: str, output_dir: str):
    ...
    # Text Generation: TXT -> PPTX
        ...
        if output_format == 'pptx':
            ...
            tmp_dir = str(Path(output_dir) / f"_tmp_pres_{Path(input_path).stem}")
            Path(tmp_dir).mkdir(parents=True, exist_ok=True)
            tmp_pptx = str(Path(tmp_dir) / f"{Path(input_path).stem}.pptx")
            prs.save(tmp_pptx)
            convert_with_libreoffice(tmp_pptx, 'odp', tmp_dir)
            tmp_odp = str(Path(tmp_dir) / f"{Path(input_path).stem}.odp")
